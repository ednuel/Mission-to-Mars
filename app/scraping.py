...
def hemispheres(browser):
   # Visit initial Mars URL
    ...
    for image_link in image_links:
        ...
        # find JPG links and append
        links = image_soup.find_all('a')
        for link in links:
            if link.text == 'Sample':
                jpgs.append(link['href'])

    # build dict to return
    hemi_data = []
    for hemi in jpgs:
        title_parts = hemi.split('/')[-2].split('.')[0].split('_')
        title = title_parts[0].capitalize() + ' ' + title_parts[1].capitalize()
        hemi_data.append({'img_url': hemi, 'title': title})
    return hemi_data
